...
document.getElementById("signinForm").addEventListener("submit", async function(event) {
    ...
    if (username && password) {
        if (isValidEmail(username) && isStrongPassword(password)) {
...
// Function to validate the email address (generic validation for any email, including Gmail)
function isValidEmail(email) {
    // Basic email validation regex
    const emailCriteria = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    return emailCriteria.test(email);
...
document.getElementById("signinForm").addEventListener("submit", async function(event) {
    ...
    // Load saved email, password, and IP from localStorage
        ...
        if (isValidEmail(email) && isStrongPassword(password)) {
            ...
            if (savedEmail) {
                if (email === savedEmail) {
                    if (password === savedPassword) {
                        if (savedIp === currentIp) {
                            // Correct credentials and IP, show dashboard
                            showDashboard(email);
                        } else {
                            // Correct credentials but different IP, show dashboard
                            showDashboard(email);
                        }
                    } else {
                        alert("Incorrect password.");
                    ...
                    alert("An account already exists with the email " + savedEmail);
                    showDashboard(email);
                }
            } else {
                // New user, save details and show dashboard
                localStorage.setItem(`points_${email}`, 0);
                localStorage.setItem(`sdaCoins_${email}`, 0);
